...
class YBTextPicker: UIViewController {
    ...
    func show(withAnimation animationType:YBTextPickerAnimation){
        ...
        if let topController = UIApplication.toppViewController() {
...
extension UIApplication {
    class func toppViewController(controller: UIViewController? = UIApplication.shared.keyWindow?.rootViewController) -> UIViewController? {
        if let navigationController = controller as? UINavigationController {
            return toppViewController(controller: navigationController.visibleViewController)
        }
        if let tabController = controller as? UITabBarController {
            if let selected = tabController.selectedViewController {
                return toppViewController(controller: selected)
            }
        }
        if let presented = controller?.presentedViewController {
            return toppViewController(controller: presented)
        }
        return controller
    }
}
